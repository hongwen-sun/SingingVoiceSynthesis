...
if test "$#" -ne 3; then
    echo "################################"
    echo "Usage:"
    echo "./02_prepare_conf_files.sh <path_to_global_conf_file> architecture synth_type"
    echo ""
...
./scripts/prepare_config_files.sh $global_config_file $architecture

echo "preparing config files for synthesis..."
./scripts/prepare_config_files_for_synthesis.sh $global_config_file $architecture $synth_type
